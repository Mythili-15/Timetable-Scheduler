...
def add_instructor(request):
    ...
    context = {
        'form': form 
        # This line creates a context dictionary that contains the form variable, which will be passed to the template for rendering.
    }
    return render(request, 'adins.html', context)
# If the request method is not POST or if the form data is not valid, this line renders the 'adins.html' 
# template with the provided context. The template will be responsible for displaying the form and any validation errors.

def inst_list_view(request): #to display instructor
    context = {
        'instructors': Instructor.objects.all() #it fetch from data base
    }
